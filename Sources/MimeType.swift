    ...
    public static let all: [MimeType] = [
        ...
        MimeType(
            ...
            matches: { bytes, swime in
                let requiredEntries = ["word/document.xml"]
                guard let zipHeader = swime.zipHeader else {
                    return false
                }
                for entry in requiredEntries {
                    if !zipHeader.entries.contains(entry) {
                        return false
                    }
                }
        ...
        MimeType(
            ...
            bytesCount: 24,
            matches: { bytes, swime in
                let requiredEntries = ["[Content_Types].xml", "_rels/.rels", "xl/workbook.xml"]
                guard let zipHeader = swime.zipHeader else {
                    return false
                }
                for entry in requiredEntries {
        ...
        MimeType(
            ...
            bytesCount: 24,
            matches: { bytes, swime in
                let requiredEntries = ["ppt/presentation.xml"]
                guard let zipHeader = swime.zipHeader else {
                    return false
                }
                for entry in requiredEntries {
